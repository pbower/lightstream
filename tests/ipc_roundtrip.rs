...
mod integration {
    ...
    fn build_all_types_table(n: usize) -> Table {
        ...
        let cat_u16 = CategoricalArray::new(
            minarrow::Buffer::from(Vec64::from_slice(
                &(0..n as u16).map(|i| (2 - (i % 3)) as u16).collect::<Vec<_>>()
            )),
            Vec64::from(vec!["foo".to_string(), "bar".to_string(), "baz".to_string()]),
            None
        );
        #[cfg(feature = "extended_categorical")]
        ...
        Table { cols, n_rows: n, name: "test".to_owned() }
    }
    ...
    async fn roundtrip_ipc(mode: IPCMessageProtocol, n: usize) {
        ...
        let schema: Vec<Field> = table.cols.iter().map(|c| c.field.as_ref().clone()).collect();
        ...
        let stream =
            DiskByteStream::open(&path, BufferChunkSize::Custom(128 * 1024)).await.unwrap();
    ...
    async fn roundtrip_ipc_with_nulls(fmt: IPCMessageProtocol, n: usize) {
        ...
        cols.push(FieldArray::new(
            Field::new("catn", ArrowType::Dictionary(CategoricalIndexType::UInt32), true, None),
            Array::TextArray(TextArray::Categorical32(Arc::new(cat_arr)))
        ));

        let table = Table { cols, n_rows: n, name: "nulls".into() };
    ...
    async fn test_file_roundtrip_via_disk() {
        ...
        let p = dir.path().join("arrow.bin");
        {
            ...
            let mut writer : TableStreamWriter<Vec64<u8>> = TableStreamWriter::new(schema, IPCMessageProtocol::File);
        ...
        let stream = DiskByteStream::open(&p, BufferChunkSize::Custom(256 * 1024)).await.unwrap();
